    ...
    do {
        ...
        for (i = 0; i < SEV_CERT_MAX_SIGNATURES; i++) {
            if ((parent_cert->pub_key_algo == SEV_SIG_ALGO_RSA_SHA256) ||
                ...
                if (EVP_DigestVerifyInit(verify_md_ctx, &verify_ctx, (parent_cert->pub_key_algo == SEV_SIG_ALGO_RSA_SHA256) ? EVP_sha256() : EVP_sha384(), NULL, parent_signing_key) <= 0) {
                ...
                found_match = true;
                cout << "SEV RSA verification Succesful" << endl;
...
SEV_ERROR_CODE SEVCert::compile_public_key_from_certificate(const sev_cert *cert, EVP_PKEY *evp_pub_key)
{
    ...
    do {
        if ((cert->pub_key_algo == SEV_SIG_ALGO_RSA_SHA256) ||
            ...
            if (EVP_PKEY_get_base_id(evp_pub_key) != EVP_PKEY_RSA) {
